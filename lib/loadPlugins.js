...
const TMP_PATTERN = /\.tmp-[\w-]+\.mjs$/
const IS_BUN = typeof Bun !== 'undefined'

function getReloadNonce() {
  return `${Date.now()}-${crypto.randomBytes(4).toString('hex')}`
}

async function importFreshNode(fullPath, nonce) {
  const fileUrl = `${pathToFileURL(fullPath).href}?v=${nonce}`
  return import(fileUrl)
}

async function importFreshBun(fullPath, nonce) {
  const dir = path.dirname(fullPath)
  const base = path.basename(fullPath, '.js')
  const tmpPath = path.join(dir, `.${base}.tmp-${nonce}.mjs`)
...
async function importFresh(fullPath, nonce) {
  return IS_BUN ? importFreshBun(fullPath, nonce) : importFreshNode(fullPath, nonce)
}

async function collectPlugins(dir, baseDir = dir, allPlugins = [], nonce) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await collectPlugins(fullPath, baseDir, allPlugins, nonce)
    } else if (entry.isFile() && entry.name.endsWith('.js') && !TMP_PATTERN.test(entry.name)) {
      try {
        const mod = await importFresh(fullPath, nonce)
...
export async function loadPlugins(dir, onProgress = null) {
  ...
  const nonce = getReloadNonce()
  const allPlugins = await collectPlugins(dir, dir, [], nonce)
